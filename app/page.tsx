...
export default function Home() {
  ...
  async function loadEmployees() {
    ...
    setEmployees((data || []).map((e) => ({ ...e, hourly_cost: Number(e.hourly_cost), hours_worked: Number(e.hours_worked), value_generated: Number(e.value_generated) })))
  }
  ...
  async function loadInventory() {
    ...
    setInventory((data || []).map((i) => ({ ...i, quantity: Number(i.quantity), months_slow: Number(i.months_slow), discount: Number(i.discount) })))
  }
  ...
  async function addCompany() {
    ...
    setName(''); setPib(''); setCity(''); setMessage('Firma uspešno dodata.'); loadCompanies()
  }
  ...
  async function addTransaction() {
    ...
    const { error } = await supabase.from('transactions').insert([{ amount: finalAmount, type: txType, description: txDescription.trim(), category, date: txDate, company_id: selectedCompany }])
    if (error) return setMessage('Greška pri dodavanju transakcije: ' + error.message)
    setTxAmount(''); setTxDescription(''); setTxDate(''); setTxType('income'); setMessage(`Dodato: ${txType} €${numericAmount} / ${category}`); loadTransactions()
  }

  async function addTask() {
    if (!selectedCompany) return setMessage('Prvo izaberi firmu.')
    if (!taskTitle.trim()) return setMessage('Unesi naziv taska.')
    const { error } = await supabase.from('tasks').insert([{ company_id: selectedCompany, title: taskTitle.trim(), assigned_to: taskAssignedTo.trim() || null, due_date: taskDueDate || null, priority: taskPriority, status: 'open' }])
    if (error) return setMessage('Greška pri dodavanju taska: ' + error.message)
    setTaskTitle(''); setTaskAssignedTo(''); setTaskDueDate(''); setTaskPriority('normal'); setMessage('Task uspešno dodat.'); loadTasks()
  }

  async function addEmployee() {
    if (!selectedCompany) return setMessage('Prvo izaberi firmu.')
    if (!employeeName.trim()) return setMessage('Unesi ime radnika.')
    const { error } = await supabase.from('employees').insert([{ company_id: selectedCompany, name: employeeName.trim(), hourly_cost: Number(hourlyCost) || 0, hours_worked: Number(hoursWorked) || 0, value_generated: Number(valueGenerated) || 0 }])
    if (error) return setMessage('Greška pri dodavanju radnika: ' + error.message)
    setEmployeeName(''); setHourlyCost(''); setHoursWorked(''); setValueGenerated(''); setMessage('Radnik dodat.'); loadEmployees()
  }

  async function addInventoryItem() {
    if (!selectedCompany) return setMessage('Prvo izaberi firmu.')
    if (!itemName.trim()) return setMessage('Unesi naziv artikla.')
    const { error } = await supabase.from('inventory_items').insert([{ company_id: selectedCompany, name: itemName.trim(), quantity: Number(itemQty) || 0, months_slow: Number(itemSlowMonths) || 0, discount: 20 }])
    if (error) return setMessage('Greška pri dodavanju artikla: ' + error.message)
    setItemName(''); setItemQty(''); setItemSlowMonths(''); setMessage('Artikal dodat.'); loadInventory()
  }

  async function addInvoice() {
    if (!selectedCompany) return setMessage('Prvo izaberi firmu.')
    if (!invoiceClient.trim() || !invoiceAmount.trim()) return setMessage('Unesi klijenta i iznos fakture.')
    const { error } = await supabase.from('invoices').insert([{ company_id: selectedCompany, client: invoiceClient.trim(), amount: Number(invoiceAmount) || 0, due_date: invoiceDue || null, status: 'pending' }])
    if (error) return setMessage('Greška pri dodavanju fakture: ' + error.message)
    setInvoiceClient(''); setInvoiceAmount(''); setInvoiceDue(''); setMessage('Faktura dodata.'); loadInvoices()
  }
  ...
  async function handleCommand(command: string) {
    ...
    if (!text) return
    const lower = text.toLowerCase()
    const looksLikeTask = lower.includes('task') || lower.includes('podseti') || lower.includes('remind') || lower.includes('zadatak') || lower.includes('uradi') || lower.includes('ponuda') || lower.includes('posalji') || lower.includes('pošalji')
    const amountMatch = text.match(/-?\d+(\.\d+)?/)
    const amount = amountMatch ? Math.abs(Number(amountMatch[0])) : 0

    if (looksLikeTask && !amount) {
      const p = parseTaskCommand(text)
      const { error } = await supabase.from('tasks').insert([{ company_id: selectedCompany, title: p.title, assigned_to: p.assignedTo, due_date: p.dueDate || null, priority: p.priority, status: 'open' }])
      if (error) return setMessage('Greška pri unosu taska: ' + error.message)
      setMessage(`Task created: ${p.title}`); loadTasks(); return
    }

    if (!amount || Number.isNaN(amount)) return setMessage('Nisam našao iznos. Primer: platila 300 za marketing ili Ana ponuda za Metalac do petka high priority')

    const isExpense = lower.includes('spent') || lower.includes('paid') || lower.includes('expense') || lower.includes('trošak') || lower.includes('trosak') || lower.includes('platio') || lower.includes('platila') || lower.includes('kupila') || lower.includes('kupili') || lower.includes('rashod')
    const isIncome = lower.includes('received') || lower.includes('income') || lower.includes('revenue') || lower.includes('uplata') || lower.includes('primili') || lower.includes('zaradili') || lower.includes('prihod')
    const type = isExpense ? 'expense' : isIncome ? 'income' : 'income'
    const finalAmount = type === 'expense' ? -amount : amount
    const category = detectCategory(lower)
    const { error } = await supabase.from('transactions').insert([{ company_id: selectedCompany, amount: finalAmount, type, description: text, category, date: new Date().toISOString().slice(0, 10) }])
    if (error) return setMessage('Greška pri unosu: ' + error.message)
    setMessage(`Dodato: ${type} €${amount} / ${category}`); loadTransactions()
  }
  ...
  const recentActivity = [
    ...filteredTransactions.slice(0, 5).map(t => ({ id: `tx-${t.id}`, label: `${t.type === 'expense' || t.amount < 0 ? 'Expense' : 'Income'} €${Math.abs(t.amount)}`, meta: `${t.date} • ${t.category || 'other'} • ${t.description}` })),
    ...filteredTasks.slice(0, 5).map(t => ({ id: `task-${t.id}`, label: `Task ${t.status === 'done' ? 'completed' : 'created'}: ${t.title}`, meta: `${t.assigned_to || 'Unassigned'} • ${t.priority || 'normal'} priority • ${t.due_date || 'no due date'}` })),
  ].slice(0, 8)

  return (
    <div style={{ padding: 24, fontFamily: 'Arial, sans-serif', background: '#000', minHeight: '100vh', color: 'white' }}>
      <h1 style={{ marginBottom: 24, fontSize: 32 }}>BASAL — Company Operating Assistant</h1>

      {message && <div style={{ ...cardStyle, maxWidth: 720 }}>{message}</div>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 24, alignItems: 'start' }}>
        <div>
          ...
          <Panel title="Companies">
            {companies.map(c => (
              <div key={c.id} onClick={() => setSelectedCompany(c.id)} style={{ marginBottom: 10, cursor: 'pointer', padding: 14, border: '1px solid #333', background: selectedCompany === c.id ? '#10161a' : '#050505', color: selectedCompany === c.id ? '#00e5ff' : 'white', borderRadius: 10 }}>
                <div style={{ fontWeight: 'bold', marginBottom: 4 }}>{c.name}</div>
                <div style={{ color: selectedCompany === c.id ? '#9cf6ff' : '#bbb' }}>{c.city} — {c.pib}</div>
              </div>
            ))}
          </Panel>
        </div>
        ...
        <div>
          {selectedCompanyData ? (
            <>
              <Panel title={`${selectedCompanyData.name} — overview`}>
                <div style={{ color: '#999' }}>{selectedCompanyData.city} • PIB {selectedCompanyData.pib}</div>
              </Panel>
              ...
              <Panel title="Voice Command Executive">
                <input placeholder='Type/voice-style command: "Ana ponuda za Metalac do petka high priority"' onKeyDown={async e => { if (e.key === 'Enter') { await handleCommand(e.currentTarget.value); e.currentTarget.value = '' } }} style={inputStyle} />
                <div style={{ color: '#777' }}>Voice API later. For now this is the same executive command layer, free and fast.</div>
              </Panel>
              ...
              <Panel title="Basal warnings">
                {warnings.map((w, i) => <div key={i} style={warningStyle}>⚠️ {w}</div>)}
              </Panel>

              <Panel title="Tasks system">
                ...
                <List>{filteredTasks.map(task => <div key={task.id} style={taskStyle}><button onClick={() => deleteTask(task.id)} style={smallDeleteButtonStyle}>X</button><b style={{ textDecoration: task.status === 'done' ? 'line-through' : 'none' }}>{task.title}</b><div style={muted}>{task.assigned_to || 'Unassigned'} • {task.due_date || 'No due date'} • {task.priority || 'normal'} • {task.status || 'open'}</div><button onClick={() => toggleTaskStatus(task)} style={secondaryButtonStyle}>{task.status === 'done' ? 'Reopen' : 'Mark done'}</button></div>)}</List>
              </Panel>
              ...
              <Panel title="Employee Trust & Performance Score">
                ...
                <List>{filteredEmployees.map(e => {
                  const cost = e.hourly_cost * e.hours_worked
                  const profit = e.value_generated - cost
                  return <div key={e.id} style={taskStyle}><b>{e.name}</b><div style={muted}>Cost: €{cost} • Value: €{e.value_generated} • Result: €{profit}</div></div>
                })}</List>
              </Panel>

              <Panel title="Ghost Marketer — Inventory Loop">
                ...
                <List>{filteredInventory.map(i => <div key={i.id} style={taskStyle}><b>{i.name}</b><div style={muted}>Qty: {i.quantity} • Slow: {i.months_slow} months</div>{i.months_slow >= 3 && <div style={insightStyle}>Ghost Marketer: “{i.name} is slow. Suggest -{i.discount}% campaign today.”</div>}</div>)}</List>
              </Panel>

              <Panel title="One-Tap Factoring / Instant Cash">
                ...
                <List>{pendingInvoices.map(i => <div key={i.id} style={taskStyle}><b>{i.client}</b><div style={muted}>€{i.amount} • due {i.due_date || 'unknown'} • {i.status}</div></div>)}</List>
              </Panel>
              ...
              <Panel title="Spending by category">
                {categoryTotals.length === 0 ? <div style={muted}>No expense data yet.</div> : categoryTotals.map(c => {
                  ...
                  return <div key={c.category} style={{ marginBottom: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>{c.category}</span><span>€{c.total} ({p}%)</span></div><div style={{ height: 6, background: '#222', borderRadius: 6, overflow: 'hidden', marginTop: 4 }}><div style={{ width: `${p}%`, height: '100%', background: '#00e5ff' }} /></div></div>
                })}
              </Panel>

              <Panel title="Company timeline">
                {recentActivity.length === 0 ? <div style={muted}>No activity yet.</div> : recentActivity.map(item => <div key={item.id} style={{ padding: '10px 0', borderBottom: '1px solid #1f1f1f' }}><b>{item.label}</b><div style={muted}>{item.meta}</div></div>)}
              </Panel>
              ...
              <Panel title="Add transaction manually">
                ...
                <select value={txType} onChange={e => setTxType(e.target.value as 'income' | 'expense')} style={inputStyle}><option value="income">Income</option><option value="expense">Expense</option></select>
              ...
              <Panel title="Transactions">
                {filteredTransactions.map(t => <div key={t.id} style={{ ...taskStyle, position: 'relative' }}><button onClick={() => deleteTransaction(t.id)} style={smallDeleteButtonStyle}>X</button><b>{t.type.toUpperCase()}</b><div>Date: {t.date}</div><div>Amount: €{t.amount}</div><div>Description: {t.description}</div><div>Category: {t.category || 'other'}</div></div>)}
              </Panel>
            </>
          ) : <Panel title="Select company"><div style={muted}>Izaberi firmu da vidiš company operating system.</div></Panel>}
        </div>
      </div>
    </div>
  )
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return <div style={{ ...cardStyle, maxWidth: 820 }}><h3 style={{ marginTop: 0, marginBottom: 16 }}>{title}</h3>{children}</div>
}

function MetricCard({ title, value, color }: { title: string; value: string; color: string }) {
  return <div style={cardStyle}><div style={muted}>{title}</div><div style={{ fontSize: 34, fontWeight: 'bold', color }}>{value}</div></div>
}
